...
class GUI:
    def __init__(self):
        self.window = Tk()
        self.startupCheck = IntVar()
        self.window.geometry('700x330')
        self.window.config(background="White")

        self.window.title("BhavCopy Download Automation Settings GUI")
        self.window.iconbitmap('BhavCopyLogo.ico')

        self.window.columnconfigure(index=0, weight=1)
        self.window.rowconfigure(index=3, weight=1)

        self.heading = Label(self.window, text="BhavCopy Download Automation Settings", relief='raised',
                             background="White")
        self.sampleheading = Label(self.window, text="\nSample Downloaded NSE Bhav Copy:", background="White")
        self.treeframe = LabelFrame(self.window)
        self.browserFrame = LabelFrame(self.window)
        self.browserFrame.columnconfigure(index=0, weight=1)

        self.labelFilename = Label(self.browserFrame)
        self.labelFilename.configure(text=CheckOrignalFilePath())
        self.fileBrowserButton = Button(self.browserFrame, text='File Browser', command=BrowseFile, background="White")

        startupTrue = Checkbutton(self.window, text='Download On System Startup', variable=self.startupCheck, onvalue=1,
                                  offvalue=0,
                                  background="White")
        ...
        tv1 = ttk.Treeview(self.treeframe)
        tv1.place(relheight=1, relwidth=1)
        treescrolly = Scrollbar(self.treeframe, orient="vertical", command=tv1.yview)
        treescrollx = Scrollbar(self.treeframe, orient="horizontal", command=tv1.xview)
        ...
        exitButton = Button(self.window, text='Save and Exit', command=StartupFunctionAndExit, background="White")

        # Color changes on hover from white to light gray
        changeOnHover(self.fileBrowserButton, "Light Gray", "White")
        changeOnHover(exitButton, "Light Gray", "White")

        # change to grid
        self.heading.grid(row=0, column=0, sticky='NSEW')
        self.sampleheading.grid(row=2, column=0, sticky='NSEW')
        self.treeframe.grid(row=3, column=0, sticky='EW', ipady=60)

        self.browserFrame.grid(row=4, column=0, sticky='EW')
        self.labelFilename.grid(row=0, column=0, sticky='W')
        self.fileBrowserButton.grid(row=0, column=1, sticky='E')
    ...
    def BrowseFile(self):
        ...
        self.labelFilename.configure(text=filename)  # reconfigures the Filename label with new location in GUI
        ...
        newPath.write(str(self.labelFilename.cget('text')))  # append the file with new location
        newPath.close()
...
def StartupFunctionAndExit():
    ...
    if self.startupCheck.get() == 1:  # checkbox clicked
        USER_NAME = getpass.getuser()
        startupPath = r'C:\Users\%s\AppData\Roaming\Microsoft\self.windows\Start ' \
        ...
        self.window.destroy()
    elif self.startupCheck.get() == 0:  # checkbox not clicked
        USER_NAME = getpass.getuser()
        startupPath = r'C:\Users\%s\AppData\Roaming\Microsoft\self.windows\Start ' \
                      r'Menu\Programs\Startup\BhavCopyAutoDownloader.lnk' % USER_NAME
        try:
